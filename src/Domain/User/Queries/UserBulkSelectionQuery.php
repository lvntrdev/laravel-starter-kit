 ...
 *
 *   2. ALLOW-LISTED FILTERS. Only the filters the datatable actually exposes
 *      (status, role, search) are honoured. Arbitrary keys in the client-
 *      supplied snapshot are ignored — a hostile snapshot cannot inject extra
 *      WHERE clauses or widen/narrow the set beyond the table's contract.
 *
...
class UserBulkSelectionQuery
{
     ...
     * @var string[]
     */
    private const ALLOWED_FILTERS = ['status', 'role', 'search'];
    ...
    /**
     * Flatten the snapshot into a key => value filter map, accepting both the
     * bracket-style keys the frontend produces from URLSearchParams
     * ('filter[status]') and an already-nested ['filter' => [...]] shape.
     * Only allow-listed filter keys survive.
     *
    ...
    private function normalizeFilters(array $snapshot): array
    {
        ...
        $filters = [];
        foreach (self::ALLOWED_FILTERS as $allowed) {
            if (array_key_exists($allowed, $flat) && is_scalar($flat[$allowed])) {
                $value = trim((string) $flat[$allowed]);
                if ($value !== '') {
                    $filters[$allowed] = $value;
                }
            }
        }
