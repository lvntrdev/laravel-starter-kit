<script setup lang="ts">
    ...
    interface Props {
        /**
         * Başlangıç raporu. Backend index() hâlâ report döndürüyorsa bu dolu gelir;
         * backend lazy-load moduna geçtiğinde null gelebilir — onMounted runChecks() devralır.
         */
    ...
    /**
     * Sayfa açıldığında rapor boş geliyorsa (backend lazy mod) veya hiç gelmiyorsa
     * otomatik olarak run endpoint'i tetiklenir.
     */
    onMounted(() => {
        if (!props.report) {
            runChecks();
    ...
    const summaryCards = computed(() => [
        {
            ...
            value: props.report?.summary.ok ?? 0,
        ...
        },
        {
            ...
            value: props.report?.summary.warn ?? 0,
        ...
        },
        {
            ...
            value: props.report?.summary.fail ?? 0,
...
<template>
    <AdminLayout
        :title="$t('sk-system-health.title')"
        :subtitle="$t('sk-system-health.subtitle')"
    >
        <div class="w-full space-y-5">
            <!-- Yükleniyor durumu — report henüz gelmediyse -->
            <div
                v-if="running && !report"
                    ...
                    {{
                        report
                            ? $t('sk-system-health.generated_at', {
                                time: formatGeneratedAt(report.generated_at),
                              })
                            : ''
                    }}
                </p>
            ...
            >
                <!-- Boş durum -->
                <div
                    v-if="!report?.checks || report.checks.length === 0"
                    class="flex flex-col items-center gap-3 py-16 text-surface-400 dark:text-surface-500"
                >
                ...
                <div v-else class="overflow-x-auto">
                    <table class="w-full">
                        <thead class="border-b border-surface-200 bg-surface-50 dark:border-surface-700 dark:bg-surface-800/50">
                            <tr class="text-left text-xs font-medium uppercase tracking-wider text-surface-500 dark:text-surface-400">
