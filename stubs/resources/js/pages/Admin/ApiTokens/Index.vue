<script setup lang="ts">
    ...
    // ── Token (access_token) modal state ──────────────────────────────────────
...
<template>
    <AdminLayout :title="$t('sk-api-tokens.title')" :subtitle="$t('sk-api-tokens.subtitle')">
        <template v-if="can('api-tokens.create')" #page-actions>
            <Button :label="$t('sk-api-tokens.create')" icon="pi pi-plus" @click="openCreateModal" />
        </template>

        <SkDatatable :config="tableConfig" :refresh-key="REFRESH_KEY" />

        <!-- Token oluşturma modal'ı -->
        <CreateTokenModal
            :visible="createModal"
        ...
        <!-- Tek sefer token gösterme modal'ı — click-outside ve ESC kapalı -->
        <OneTimeSecretModal
