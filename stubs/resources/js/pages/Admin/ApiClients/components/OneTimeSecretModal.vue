<script setup lang="ts">
    ...
    /**
     * Tek sefer gizli anahtar gösterme modal'ı.
     *
     * Güvenlik kısıtları:
     *   - closable: false → X butonu yok
     *   - dismissableMask: false → dışarı tıklama kapatmaz
     *   - closeOnEscape: false → ESC tuşu kapatmaz
     *   - "Sakladım" butonuna basılmadan kapanmaz
     */

    interface Props {
        visible: boolean;
        secret: string;
        /** 'secret' | 'token' — i18n key prefix'ini belirler */
        type?: 'secret' | 'token';
    }
    ...
    const titleKey = computed(() =>
        props.type === 'token' ? 'sk-api-tokens.token_modal.title' : 'sk-api-clients.secret_modal.title',
    );
...
<template>
    ...
    >
        ...
        <div class="mb-5">
            ...
            >
                <code
                    class="flex-1 break-all font-mono text-base text-surface-800 select-all dark:text-surface-100"
                >{{ secret }}</code>
                <Button
