...
- `toolbar('minimal' | 'standard' | 'full')` — toolbar layout. `minimal` covers bold / italic / underline, text color and highlight; `standard` adds strikethrough, bullet / numbered / task lists, quote, alignment (incl. justify), clear formatting and tables; `full` adds H2–H4, inline code, sub/superscript, font family / size / line height, code block, divider, YouTube embed and undo / redo. Every preset has the **HTML source** toggle (edit the markup directly — every keystroke reaches the form value, so submitting with the source view open keeps the edit; tags the editor cannot store are listed under the textarea while you type) and a **fullscreen** toggle (Esc leaves). The link and image buttons follow `links()` / `imageUpload()`. Default `'standard'`.
