...
SSR support ships with the application and is wired through the same Inertia/Vite entrypoint. Runtime enablement is controlled by `INERTIA_SSR_ENABLED` in `config/inertia.php`; when it is off, the app falls back cleanly to client rendering without changing the page code.
