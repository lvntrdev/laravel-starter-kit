...
- `toolbar('minimal' | 'standard' | 'full')` — toolbar düzeni. `minimal` bold / italic / underline, yazı rengi ve vurgu rengi; `standard` üstü çizili, madde / numaralı / görev listesi, alıntı, hizalama (iki yana yaslama dahil), biçimi temizle ve tablo ekler; `full` H2–H4, satır içi kod, alt/üst simge, yazı tipi / boyut / satır aralığı, kod bloğu, ayraç, YouTube gömme ve geri al / yinele ekler. Her preset'te **HTML kaynağı** düğmesi (işaretlemeyi doğrudan düzenleme — her tuş form değerine anında yansır, kaynak açıkken gönderilse de düzenleme kaybolmaz; editörün saklayamadığı etiketler yazarken metin kutusunun altında listelenir) ve **tam ekran** düğmesi (Esc ile çıkılır) bulunur. Link ve görsel düğmeleri `links()` / `imageUpload()` ayarına bağlıdır. Varsayılan `'standard'`.
