...
- the selected interface locale is stored in a long-lived cookie (survives logout; the legacy session key is read only as a fallback)
...
| --- | --- | --- | --- |
| `POST` | `/locale` | `locale.update` | Updates the active interface locale (long-lived cookie) |
