...
SSR desteği uygulamanın içinde hazır gelir ve aynı Inertia/Vite giriş noktası üzerinden çalışır. Çalışma zamanında etkin olup olmayacağı `config/inertia.php` içindeki `INERTIA_SSR_ENABLED` ayarıyla belirlenir; kapalıyken uygulama aynı sayfa koduyla sorunsuz şekilde client render'a döner.
