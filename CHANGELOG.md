...
- **`allow_unresolved` defaults to `false` in the next minor release.** It is `true` today, so an unresolved route still passes with a warning and nothing in this release changes that behavior. See the [upgrade guide](docs/UPGRADE.md#deprecation--unresolved-route-fail-closed-default-flips-in-the-next-minor) for the remediation path to run before the flip.
